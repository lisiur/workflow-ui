import {
    ...
    CanvasDblClickEvent, CanvasDragEndEvent,
    CanvasDragEnterEvent,
    CanvasDragEvent,
    CanvasDragLeaveEvent,
    CanvasDragOverEvent,
    CanvasDragStartEvent, CanvasDropEvent,
    CanvasEvent, CanvasGlobalMouseMoveEvent,
...
export class WidgetStateMachine {
    ...
    static dragPayload = {
        ...
        origin: null as Widget | null,
    }

    // 状态机
    private stateMachine: StateMachine<WidgetState, WidgetEvent>
    // 鼠标位置
    private prevPointerPosition: Vec2
    // 鼠标位置是否在 widget 内
    private prevPointerIncludeInWidget: boolean

    constructor(public widget: Widget) {
        this.widget = widget
        this.prevPointerIncludeInWidget = false
        this.prevPointerPosition = new Vec2(0, 0)
        const sm = new StateMachine<WidgetState, WidgetEvent>(WidgetState.Normal)
    ...
    handleEvent(event: CanvasEvent): null | CanvasEvent {
        ...
        let outputEvent: CanvasEvent | null = null
